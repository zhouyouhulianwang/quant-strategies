...
def test_login(role, username, password):
    try:
        ...
        if data['status'] == 'success':
            ...
            reporter.add_test(f'Login as {role}', 'PASS', f"Token: {token[:20]}..., Role: {actual_role}", role)
            return token
        else:
            reporter.add_test(f'Login as {role}', 'FAIL', data.get('message', 'Unknown error'), role)
            return None
    except Exception as e:
...
def test_market_data(token, role):
    try:
        resp = requests.get(f"{BASE_URL}/api/v2/market/AAPL", headers={'Authorization': f'Bearer {token}'}, timeout=5)
        data = resp.json()
        assert data['status'] == 'success'
        reporter.add_test('Market Data (AAPL)', 'PASS', f"Price: ${data['data']['price']:.2f}", role)
    except Exception as e:
        reporter.add_test('Market Data (AAPL)', 'FAIL', str(e), role)

def test_portfolio(token, role):
    try:
        resp = requests.get(f"{BASE_URL}/api/v2/portfolio", headers={'Authorization': f'Bearer {token}'}, timeout=5)
        data = resp.json()
        assert data['status'] == 'success'
        reporter.add_test('Portfolio Access', 'PASS', f"Positions: {len(data['data']['positions'])}, Total: ${data['data']['total_value']:,.2f}", role)
    except Exception as e:
    ...
    try:
        ...
        if should_succeed:
            if resp.status_code == 200:
                data = resp.json()
                reporter.add_test('Create Order', 'PASS', f"Order ID: {data['data']['order_id']}", role)
            else:
                reporter.add_test('Create Order', 'FAIL', f"Expected 200, got {resp.status_code}", role)
        else:
            if resp.status_code == 403:
                reporter.add_test('Create Order (Blocked)', 'PASS', "Correctly blocked with 403", role)
            else:
                reporter.add_test('Create Order (Blocked)', 'FAIL', f"Expected 403, got {resp.status_code}", role)
    except Exception as e:
...
def test_unauthorized_access():
    try:
        ...
        if resp.status_code == 403:
            reporter.add_test('Unauthorized Access', 'PASS', "Correctly blocked with 403")
        else:
            reporter.add_test('Unauthorized Access', 'FAIL', f"Expected 403, got {resp.status_code}")
    except Exception as e:
...
def test_browser_redirect():
    try:
        ...
        resp = requests.get(f"{BASE_URL}/api/v2/portfolio", headers=headers, allow_redirects=False, timeout=5)
        
        if resp.status_code == 302 and '/login' in resp.headers.get('Location', ''):
            reporter.add_test('Browser Redirect', 'PASS', f"Redirected to {resp.headers.get('Location')}")
        else:
            reporter.add_test('Browser Redirect', 'FAIL', f"Expected 302 redirect, got {resp.status_code}")
    except Exception as e:
...
def test_login_page():
    try:
        ...
        if resp.status_code == 200 and 'QuantAlpha' in resp.text:
            reporter.add_test('Login Page', 'PASS', "Page loaded with QuantAlpha branding")
        else:
            reporter.add_test('Login Page', 'FAIL', f"Status: {resp.status_code}")
    except Exception as e:
