...
class AdaptiveMomentumStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        # ============ 多因子参数(已禁用,保留配置兼容性)============
        self.multi_factor_enabled = False  # 禁用多因子
    ...
    def _BuildSectorMap(self) -> Dict[str, str]:
        """构建行业映射表 - 从 strategy_config.py 加载 SECTOR_MAP"""
        try:
            from strategy_config import SECTOR_MAP
            self.Log(f"从 strategy_config.py 加载行业映射: {len(SECTOR_MAP)} 只股票")
            return SECTOR_MAP
        except Exception as e:
            self.Log(f"加载 strategy_config.py 失败: {e}")
        
        # Fallback: 尝试读取 JSON（本地回测兼容）
