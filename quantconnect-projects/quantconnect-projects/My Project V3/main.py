"""
策略名称：AdaptiveMomentumStrategy v3.0
描述：基于多期限动量和行业轮动的自适应量化策略
优化项：
  - 动态调仓频率
  - 自适应权重调整
  - 行业轮动过滤
  - 多层风险管理（止损、回撤保护、波动率缩放）
  - 趋势与RSI过滤
  - 执行优化
"""
...
class AdaptiveMomentumStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        # ============ 仓位管理 ============
        self.max_position_pct = 0.15  # 单票最大仓位
        self.min_position_pct = 0.00  # 取消最小仓位限制
        self.max_stocks = 10
        self.min_score = 0.0
        self.max_total_exposure = 0.80  # 总仓位上限
        self.min_total_exposure = 0.20  # 总仓位下限
        ...
        self.max_drawdown_pct = 0.25  # 最大回撤保护
    ...
    def RebalanceMarket(self, market_symbols: Dict[str, Symbol], market_name: str):
        ...
        if total_weight > self.max_total_exposure:
            ...
            self.Log(f"  总仓位{total_weight*100:.1f}%>{self.max_total_exposure*100:.0f}%, 等比例缩减")
