...
class AdaptiveMomentumStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        # ============ VIX参数 ============
        # 使用VIX指数替代VIXY（VIXY是ETF，有衰减问题）
        self.vix_symbol = self.AddIndex("VIX", Resolution.DAILY).Symbol
        self.vix_threshold = 25.0  # VIX恐慌阈值（正常10-20，>25为压力）
        self.vix_pause_level = 25.0
        self.vix_boost_level = 15.0
    ...
    def _HedgeWithShortSPY(self):
        ...
        for symbol in self.Portfolio.Keys:
            if symbol.SecurityType == SecurityType.Equity:
                ticker = self.GetTickerName(symbol)
                if ticker not in self.safe_tickers and ticker not in ["SPY", "QQQ", "IWM", "VIX"]:
