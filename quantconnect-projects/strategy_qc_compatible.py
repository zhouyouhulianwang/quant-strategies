...
class AdaptiveMomentumStrategy(QCAlgorithm):
    def Initialize(self):
        ...
        self.Schedule.on(self.DateRules.every(DayOfWeek.Monday), self.TimeRules.after_market_open("SPY",5), self.weekly_update)
        self.Schedule.on(self.DateRules.every(DayOfWeek.Tuesday), self.TimeRules.after_market_open("SPY",30), self.weekly_hk_update)
        self.Schedule.on(self.DateRules.everyDay("SPY"), self.TimeRules.after_market_open("SPY",60), self.check_stop_loss)
    ...
    def WeeklyUpdate(self):
        self.week_counter += 1
        self.adjust_rebalance_frequency()
    ...
    def RebalanceUS(self):
        ...
        if self.sec_rot:
            top_sectors = self.get_sector_momentum()
            us_symbols = {}
        ...
        self.rebalance_market(us_symbols, "US")
    
    def RebalanceHK(self):
        if not self.enable_hk or self.is_warming_up:
            return
        pass
    ...
    def RebalanceMarket(self, m_sym, m_name):
        ...
        for name, symbol in m_sym.items():
            result = self.calculate_momentum_score(symbol, name)
        ...
        for symbol in list(self.cost_b.keys()):
            if symbol not in targets and self.get_ticker_name(symbol) in m_sym and self.portfolio[symbol].Invested:
                self.liquidate(symbol)
                del self.cost_b[symbol]
