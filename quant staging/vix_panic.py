...
class VixPanicSignal:
    ...
    def __init__(self, algorithm, config=None):
        ...
        self.stats = defaultdict(lambda: {
            "count": 0,
            "SPX_D0": [], "SPX_D1": [], "SPX_D2": [], 
            "SPX_D3": [], "SPX_D4": [], "SPX_D5": [],
        })
        self.trades = []
    ...
    def initialize_symbols(self, spx='SPY', dji='DIA', ndx='QQQ', vix=None):
        ...
        algo = self.algo
        self.symbols = {
            'SPX': algo.AddEquity(spx, Resolution.DAILY).Symbol,
    ...
    def use_existing_symbols(self, spx_symbol, dji_symbol, ndx_symbol, vix_symbol, rsi_indicator=None):
        ...
        self.rsi_indicator = rsi_indicator or self.algo.RSI(spx_symbol, self.config['rsi_period'], MovingAverageType.SIMPLE)
        self.algo.Log("[VIX_PANIC] Using existing symbols from main strategy")
        return self
    ...
    def get_signal(self, data=None):
        ...
        Args:
            data: 可选，传入Slice数据，否则使用algo.Securities
        ...
        # 检查是否已初始化
        if not self.symbols or not self.vix_symbol:
            return {'signal': 'none', 'reason': 'VIX_PANIC not initialized'}
    ...
    def check_position_exit(self):
        ...
        Returns:
            list: 需要平仓的标的列表，每个元素为 {'symbol': sym, 'reason': str, 'pnl': float}
        """
        algo = self.algo
        exits = []
        
        if not self.hold_long or not self.entry_date:
            return exits
    ...
    def _record_stats(self, signal):
        ...
        for d in range(6):
            rec[f"SPX_D{d}"].append(forward_ret.get('SPX', {}).get(f"D{d}", 0))
    ...
    def get_stats_report(self):
        ...
        for key in group_order:
            ...
            for idx_name in ["SPX", "DJI", "NDX"]:
                d0_avg, d0_win = self._calc_stats(data.get(f"{idx_name}_D0", []))
                d1_avg, d1_win = self._calc_stats(data.get(f"{idx_name}_D1", []))
                d5_avg, d5_win = self._calc_stats(data.get(f"{idx_name}_D5", []))
                lines.append(f"{idx_name} D0:{d0_avg}%/{d0_win}% D1:{d1_avg}%/{d1_win}% D5:{d5_avg}%/{d5_win}%")
